...
function objMainFrame:new(fParent, tTexture, oSettings, oDisplay)
	...
	-- Waypoint Arrow: Refresh arrow direction
	obj.RefreshWaypoint = function(self)
		...
		if t and t.x and t.y and t.zone then
			if VGuideWaypointArrow then
				local waypointTitle = title .. " - Step " .. step
				VGuideWaypointArrow:SetWaypoint(t.x, t.y, t.zone, waypointTitle, label)
			end
		else
			if VGuideWaypointArrow then
