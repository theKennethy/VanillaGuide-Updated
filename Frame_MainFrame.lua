...
function objMainFrame:new(fParent, tTexture, oSettings, oDisplay)
	...
	obj.RefreshStepFrameLabel = function(self)
		...
		-- Enhance with quest log status indicators
		if VGuideQuestTracker and VGuideQuestTracker.EnhanceColorizedText then
			s = VGuideQuestTracker:EnhanceColorizedText(s)
		end
	...
	obj.RefreshScrollFrame = function(self)
		...
		for k, v in pairs(fChild.Entries) do
			if k <= oDisplay:GetCurrentStepCount() then
				...
				if VGuideQuestTracker and VGuideQuestTracker.EnhanceColorizedText then
					displayText = VGuideQuestTracker:EnhanceColorizedText(displayText)
				end
	...
	-- pfQuest Integration: Refresh waypoints and quest tracking
	obj.RefreshPfQuest = function(self)
		local tPfQuest = oSettings:GetSettingsPfQuest()
		if not tPfQuest or not tPfQuest.Presence then return end
		...
		-- Set waypoint via pfQuest if coordinates exist
		if t and t.x and t.y and t.zone then
			if tPfQuest.WaypointEnable then
				obj:SetPfQuestWaypoint(t.x, t.y, t.zone, title, label)
			end
		else
			obj:ClearPfQuestWaypoint()
		end
		
		-- Track quest on map if enabled
		if tPfQuest.QuestTrackingEnable and label then
			obj:ShowPfQuestOnMap(label)
		end
	end
	...
	-- Clear pfQuest waypoint
	obj.ClearPfQuestWaypoint = function(self)
	...
	obj.ShowPfQuestOnMap = function(self, stepText)
		...
		Dv("    Showing quest on pfQuest map: " .. questName)
		
		-- Use pfBrowser to search and display quest
		if pfBrowser and pfBrowser.SearchQuest then
			pfBrowser:SearchQuest(questName)
		end
		
		-- Alternative: Use pfQuest directly
		if pfQuest and pfQuest.UpdateQuestGivers then
			-- This triggers pfQuest to refresh quest markers
			pfQuest:UpdateQuestGivers()
		end
	end
	
	-- Get pfQuest map file name from zone name
	obj.GetPfQuestMapFile = function(self, zoneName)
		if not zoneName then return nil end
		
		-- Map zone names to map file names used by pfQuest
		local zoneToMapFile = {
			-- Kalimdor
			["Durotar"] = "Durotar",
			["Mulgore"] = "Mulgore",
