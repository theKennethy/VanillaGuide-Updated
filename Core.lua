...
function VGuide:OnEnable(first)
    ...
    self.previousQuestCount = numQuests or 0

    Dv("      -- Event |c00FF3333OnEnable|r:  Player is logged in...loading UI!")
    --Dv("    -- OnEnable End")
end

-- Called when quest log changes - refresh display to update status indicators
function VGuide:OnQuestLogUpdate()
    -- Invalidate quest tracker cache
    if VGuideQuestTracker and VGuideQuestTracker.InvalidateCache then
        VGuideQuestTracker:InvalidateCache()
    end
    
    -- Check for auto-advance conditions
    if VGuideAutoAdvancer then
        -- Check for newly accepted quests
        VGuideAutoAdvancer:CheckForAcceptedQuests(self.previousQuestCount)
        -- Check for completed/turned in quests
        VGuideAutoAdvancer:CheckForCompletedQuests()
    end
    
    -- Update previous quest count
    local _, numQuests = GetNumQuestLogEntries()
    self.previousQuestCount = numQuests or 0
